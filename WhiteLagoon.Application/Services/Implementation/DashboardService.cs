...
namespace LibraryBook.Application.Services.Implementation
{
    public class DashboardService : IDashboardService
    {
        ...
        {
            var bookingData = _unitOfWork.Booking.GetAll(u => DateOnly.FromDateTime(u.BookingDate) >= DateOnly.FromDateTime(DateTime.Now.AddDays(-30)) &&
              DateOnly.FromDateTime(u.BookingDate) <= DateOnly.FromDateTime(DateTime.Now))
            ...
            var customerData = _unitOfWork.User.GetAll(u => u.CreatedAt >= DateOnly.FromDateTime(DateTime.Now.AddDays(-30)) &&
            u.CreatedAt <= DateOnly.FromDateTime(DateTime.Now))
        ...
        public async Task<RadialBarChartDto> GetRegisteredUserChartData()
        {
            ...
            var countByCurrentMonth = totalUsers.Count(u => u.CreatedAt >= DateOnly.FromDateTime(currentMonthStartDate) &&
            u.CreatedAt <= DateOnly.FromDateTime(DateTime.Now));

            var countByPreviousMonth = totalUsers.Count(u => u.CreatedAt >= DateOnly.FromDateTime(previousMonthStartDate) &&
            u.CreatedAt <= DateOnly.FromDateTime(currentMonthStartDate));
